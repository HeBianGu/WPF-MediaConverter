        ...
        public override void CreateArguments(FFMpegArgumentOptions options)
        {
            options.WithVideoCodec(Codec)
                               ...
                               .WithVideoFilters(filterOptions =>
                               {
                                   ...
                                   if (this.UseDrawText && !string.IsNullOrEmpty(this.Text))
                                   {
                                       filterOptions
                                       .DrawText(DrawTextOptions
                                       .Create(this.Text, "")
                                       .WithParameter("fontcolor", this.Fontcolor)
                                       .WithParameter("fontsize", this.FontSize.ToString())
                                       .WithParameter("box", this.Box ? "1" : "0")
                                       .WithParameter("boxcolor", this.Boxcolor)
                                       .WithParameter("boxborderw", this.Boxborderw.ToString())
                                       .WithParameter("x", this.X)
                                       .WithParameter("y", this.Y));
                                   }
